...
### Stage 7 — Live verification — deferred (not skipped)
*Addresses: goal 6's "test the bot live on Discord."*

**Reordered behind Stage 8, on purpose.** This stage requires a real Discord application, `DEPLOY_ENABLED`, and a human watching Discord respond — it cannot be delegated to an agent in this environment. Rather than leave it as a placeholder, the plan defers it explicitly: the maintainer will run this verification by hand against their own Cloudflare and Discord accounts once Stage 8 gives the template its own identity (so the account they verify against is provisioned under the template's real name, not the inherited generic one). Expect this pass to surface documentation gaps and corrections; those land as follow-up commits, not silently.

**This verification happens in a project created *from* the template, never in `cloudflare-workers-discord-template` itself.** This repo is the upstream template, not a bot instance — it must stay with `DEPLOY_ENABLED` unset, no real Discord application wired up, and no deployed Worker, exactly as `README.md`'s "Nothing here deploys to Cloudflare or registers commands with Discord until you deliberately turn those on" promises consumers. Concretely:

1. Use **Use this template** (or fork) on GitHub to create a separate repository.
2. Clone that new repository — not this one — and follow its own README quickstart: rename the Wrangler/`package.json` identifiers, create a Discord application, fill in `.dev.vars`.
3. In that downstream repo only: enable `DEPLOY_ENABLED`, deploy to `non-prod`, register the sample command, set the Interactions Endpoint URL, and exercise the command from a real Discord server.
4. Bring anything the walkthrough surfaces — broken instructions, missing steps, unclear docs — back to this template as a follow-up commit here, not to the downstream project.

Record the outcome (and any doc corrections it surfaces) as a follow-up commit rather than closing the loop silently.

### Stage 8 — Template identity and version reset — done
*Addresses: goal 1/5's "make this repo's own identity clear," and the "include a template version" requirement in `claude.md`.*

**Decided/implemented:**

- **Reset, not continued.** `package.json`'s `name` moved from the inherited `cloudflare-workers-template` to `cloudflare-workers-discord-template`; `version` reset to `0.1.0` rather than continuing the `0.2.1` lineage (the pending changesets would otherwise have produced `1.0.0`, a number that describes the generic template's history, not this one). `description` and `keywords` updated to describe a Discord bot template instead of generic boilerplate. The three Wrangler `name` fields in `wrangler.jsonc` (and their example blocks in `README.md`/`docs/using-this-template.md`) were updated to match.
- **Pending changesets consumed through the repository's normal release tool first** (`npm run version`, i.e. `changeset version`), so the five changesets accumulated across Stages 0–5 landed in `CHANGELOG.md` with their real commit references rather than being hand-transcribed. The tool-produced `## 1.0.0` heading was then renamed to `## 0.1.0` and given a leading "Template identity reset" note explaining the rename, the version reset, and linking the fork point.
- **Fork point recorded:** this repository forked from [`mbakaitis/cloudflare-workers-template`](https://github.com/mbakaitis/cloudflare-workers-template) at tag `v0.2.1` (commit `7a33ef1b7a7a0bdab5b48c6563636d582ac98617` — confirmed via `git merge-base HEAD upstream/main`), recorded in the `CHANGELOG.md` entry per the "downstream project should be able to identify the upstream version it started from" requirement.
- **Instruction contract version: no bump.** All three instruction files were already aligned at `2.1.0` (Stage 1's bump, reaffirmed with no further change at the end of Stage 5); Stage 8's rename/version work doesn't add or change a required project-shape item, so there's nothing to bump.
- **Not yet done:** tagging the `v0.1.0` release is a shared-state action (pushes to the remote) and is left for the maintainer to trigger deliberately, after Stage 7's live verification confirms the template actually works end to end against a real Discord/Cloudflare account.
- Validation: `npm test`, `npm run lint`, and `npm install` (to resync `package-lock.json`'s `name`/`version`) all pass. Manual `grep` confirmed no remaining references to the old package name outside historical `CHANGELOG.md`/`project-approach.md` narration of earlier stages.
...
- **Stage 8 was moved ahead of Stage 7.** Stage 7 cannot be delegated to CI or to an agent — it requires a real Discord application, a real Cloudflare account, and a human watching Discord respond — so it is deferred rather than attempted here. Stage 8 (identity/version reset) doesn't depend on Stage 7's outcome, and giving the template its final name/version first means the maintainer's live verification exercises the template under its real identity instead of the inherited generic one.
...
- When to tag `v0.1.0` and push it — after Stage 7's live verification confirms the renamed/reset template actually works end to end, not before.

Resolved: Ed25519 verification approach, command-registration mechanism, npm script names, and secret naming/scope — see Stage 2. File location for the shared command-definitions module (`src/command-definitions.js`) and the Worker's signature-verification module (`src/discord-signature.js`) — see Stage 4. Package version reset vs. continuation — reset to `0.1.0`, see Stage 8.
